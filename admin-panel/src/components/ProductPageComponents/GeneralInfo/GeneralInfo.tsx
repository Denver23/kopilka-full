...
import {
    GetProductBrand,
    GetProductCategory, GetProductFeatures,
...
const GeneralInfo: React.FC<{generalForm: any}> = (props) => {
    ...
    const imagesColumns: ColumnsType<productImageTable> = [
        ...
        },
        {
            ...
            width: 50,
            render: (text: string, record: productImageTable, index: number) => {
                return <button className={s.deleteImageButton} onClick={() => {deleteImage(record._id)}}><DeleteOutlined /></button>
            }
        }
    ];
    ...
    const onCategoryBlur = () => {
        let categoriesForCheck = categories.map(item => {return item.value});
    ...
    const onBrandBlur = () => {
        let brandsForCheck = brands.map(item => {return item.value});
    ...
    return <div>
        ...
        >
            <div className={s.generalInfo}>
                ...
                >
                    <AutoComplete
                        ...
                        onBlur={onBrandBlur}
                        onSelect={(value) => {onBrandSelect(value)}}
            ...
            <Table pagination={false} columns={imagesColumns} dataSource={images.map((imageObject, index) => {return {
                _id: imageObject._id,
                key: index,
                ...
                thumbnail: imageObject.thumbnail
                   ...
                   footer={()=> {return <Button type="dashed" onClick={addImage} icon={<PlusOutlined />}>
                       Add New Image
                   </Button>}}/>
