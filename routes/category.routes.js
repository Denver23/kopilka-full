...
router.get('/:category',
    async (req, res) => {
        try {
            ...
            if (categoryDB === null) {
                res.status(406).json({errorMessage: 'Category Not Found!'});
                return;
            }
            ...
            let products = await Product.find(DBquery, {
                ...
                childProducts: 1,
                images: 1
            }).skip(page * limit).limit(limit).populate('brand');
            ...
            for (const child of nestedCategories) {
                ...
                if (child.childCategories.length > 0) {
                    childCategories = await Category.findById(child._id, {
                        childCategories: 1,
                        _id: 0
                    }).populate('childCategories', ['name', 'url']);
                } else {
                    childCategories = [];
                }
